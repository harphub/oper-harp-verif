...
fn_station_selection <- function(domain_choice,
  ...
  # SPECIFY LAT/LON BOXES OR WMO SID RANGES.
  # DO NOT USE THESE NAMES ELSEWHERE!
  #=================================================#
  
  # Note the vector concatenated with "Synop" is used to generate lists based
  # on SID ranges. All other names are assumed to be generated by lat/lon
  # bounding boxes.
  all_reserved_names <- c("DINI","IE_EN","IS","NL","NL_OP","DK","IRL","SCD",
                          "FR","DE","Alps",
                          paste0(c("France","Germany","Ireland","Norway",
  ...
  if (generate_domains) {
    ...
    for (domain in domains_to_gen) {
      ...
      #  elon <- 9
        ...
        nlat <- 48.2
        elon <- 17.3
      } else if (grepl("Synop",domain,fixed = TRUE)) { # Filter based on WMO SID range
        slat <- NULL
        wlon <- NULL
        nlat <- NULL
        elon <- NULL      
      } else if (domain == "DISPLAY_ALL_STATIONS") {
        cat("No filtering of stations applied\n")
      } else {
  ...
  if (any(names(all_station_lists) %in% names(all_gen_domains))) {
    ...
    for (dc in domain_choice) {
      if (dc %in% names(all_station_lists)) {
        ...
        # Plot
        if ((plot_domains) & (!is.null(stationlist_choice))) {
          if (!("elevmap" %in% names(stationlist_choice))) {
            psc <- allstations %>% filter(SID %in% stationlist_choice$SID)
          } else {
