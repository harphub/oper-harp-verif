...
| verif | domains      | What SID lists to verify (equivalent to SURFSELECTION/TEMPSELECTION in monitor, where the station lists are defined in scr/selection.pm). The choice "All" will use all stations available in the data. SID lists can be defined in `fn_station_selection` (see the documentation below for more information). To see the default SID lists available in the repo, use `fn_station_selection("all_domains")`. | domains: <br> -"All" <br> <br> domains: <br> - "IRL" <br> - "IE_EN" | 
