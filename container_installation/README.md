# Running harp in a container on Atos using apptainer (formerly known as singularity)
...
Once you built the image on your laptop you can push it to dockerhub
...
## Building a container on Atos

Before being able to use the image on Atos
and run a container there you need to create a Singularity Image Format (SIF).
...
## Running a container on Atos using apptainer

You can test a container on Atos using:
...
will use the R installation that includes harp in a container created with the image in `harp_0.3.sif`
