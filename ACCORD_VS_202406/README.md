...
Polly Schmederer (GeoSphere Austria), Carlos Peralta (DMI) and Fabrizio Baordo (DMI)
...
Generalising spatial verifications: 
  ...
  - Providing R scripting examples for reading, verificatio, harp panel tool usage.
...
NWP:
  ...
  Grib files output of the DEODE workflow running HARMONIE cy46h1 (total precipitation and FULL POS simulated radiances channels WV_062 & IR_108)

As part of the WP6 of the [CERISE](https://cerise-project.eu) (Copernicus Climate Change Service Evolution) project, DMI is contributing to the spatial verification of snow cover using the harp verification package.   
...
* **reading_functions (in scripts folder)**
  ...
  **using reticulate**
  ...
  - ``reading_functions.R``: Contains reading functions written in R.
...
* **panelification**
  ...
  **in scripts folder**
  ...
  - ``panel_main.R``: Main scripts for panelification. Reads configs, does the verification, calls ranking and plotting.functions.
  ...
  **in panel_configs folder**
  - ``panelification.yml``: Sets the configs for which the panel tool shall be run.
    ...
    There is a switch that allows to plot FSS and fields separately.
  ...
  - ``definitions_sat_data.R``: Collects the information needed to read and verify simulated satellite channels from DEODE experiments against SEVIRI data.
...
- To run panelification out of the box
  ...
  3. export your R local installation, e.g.:
     (in case step 2 alone is not working)
      ```
  ...
  5. run panelification for precipitation, e.g.:
   ...
   secondly, you must replace the python_version in the same definition file to the path where your python is installed.
...
**... interprete a panelification plot**

  Fo information on what you see on a panel plot, check the [interpretation help](Panel_INTERPRETATION.md).

  
**... add other models**

To add another model (panel) to panelification, a file with their definitions on how to read/verify must be added, which will then be added to panelification.yml.
...
An example using data from dini versus IFS is also provided, with the definitions in ``definitions_tp_data_dini.R`` .
To test this example use the config file in `panel_configs/panel_dini_ifs.yml` and run
     ...
     ```
Note that the IFS data contain total precipitation fields multiplied by a factor of 1e3 to 
make it comparable with the data from dini, that produces total precipitation in units of `kg m^2`, while
IFS data in `m`. The units in the IFS files are not updated. A sample script to fetch IFS data from MARS (meteorological archival and retrieval system) is included.
